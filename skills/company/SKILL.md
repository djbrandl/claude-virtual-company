...
## Playground Presentation Protocol

After any Plan-agent phase completes (Architecture, Design, Code Review), check for playground HTML files generated by those agents:

```bash
ls .company/artifacts/playground/*.html 2>/dev/null
```

If playground files are found AND the user has playgrounds enabled:

1. **Check preference**: Read `.company/state.json` `playground_preference` field
   - If `null`: Ask the user using the company-playground preference protocol
   - If `"disabled"`: Skip playground, continue to next phase
   - If `"enabled"`: Present the playground

2. **Open in browser**:
```bash
# Open the most recent playground file
PLAYGROUND=$(ls -t .company/artifacts/playground/*.html 2>/dev/null | head -1)
...
3. **Describe what it shows**: Tell the user what the playground contains and how to interact with it

4. **Ask for paste-back**:
```
AskUserQuestion({
  questions: [{
    header: "Playground",
    question: "I've opened an interactive playground in your browser. Use the controls to configure your preferences, then click 'Copy Prompt' and paste the output here. Or skip to continue without visual input.",
    options: [
      { label: "Paste Output", description: "I'll paste the playground output (use 'Other' to paste)" },
      { label: "Skip Playground", description: "Continue without visual configuration" }
    ]
  }]
})
```

5. **Apply pasted output**: Parse the `PLAYGROUND DECISIONS:` block and incorporate decisions into the workflow context

6. **Clean up**: After processing (or skipping), remove the playground file:
```bash
rm -f .company/artifacts/playground/*.html
```

**Applicable phases**:
- After Phase 4 (System Design): Check for `architecture-map.html` or `ui-design.html`
- After Phase 7 (Code Review): Check for `code-review.html`
